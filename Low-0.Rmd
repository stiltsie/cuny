...
```{r load-arbuthnot-data, eval=TRUE}
...
2.  Is there an apparent trend in the number of girls baptized over the years?  
    ...
    The number of girls getting baptized decreased for a period between 1640 and 1660, but thereafter grew for the next 40+ years only to dip again for brief spell (but only slightly compared to previous lows) in the early 18th century.
...
```{r plot-propboys-vs-year, eval=FALSE}
...
-   How do these counts compare to Arbuthnot's? Are they on a similar scale?
    ...
    The birth counts in this data set are much higher, presumably since Arbuthnot was investigating births in London in the 17th/18th centuries whereas this data sets covers all moder day births in the U.S.
...
```{r plot-boygirlratio-vs-year, eval=FALSE}
plot(present$year, present$boys/present$girls, type = "l")
```

Yes, throughout this time period, boys were born in greater proportion to girls, though the ratio has been declining over this period.
...
```{r max-boygirlratio-vs-year, eval=FALSE}
which.max(present$boys + present$girls)
```

The answer is row 22: 1961
