...
2000 cases and 9 variables. Data types as follows:
...
```{r table-gender-prop, eval=FALSE}
table(cdc$gender)
...
```{r table-exerany-prop, eval=FALSE}
table(cdc$exerany)/20000
```

```{r table-genhlth-prop, eval=FALSE}
table(cdc$genhlth)/20000
```

23% of the sample reported being in excellent health.
...
A greater proportion of men than women have smoked 100 cigarettes in their lifetime.
...
```{r under23-and-smoke, eval=FALSE}
...
4.  What does this box plot show? Pick another categorical variable from the 
    ...
    This boxplot shows that in general the greater someone's body mass index, the poorer their self   reported health.

```{r boxplot-bmi-smoker, eval=FALSE}
boxplot(bmi ~ cdc$smoke100)
```

This boxplot appears to suggest that in general there is *not* a huge difference in distribution in the BMIs for smokers vs non-smokers.  In fact there are a few outliers with large BMIs among the non-smokers and a few outliners with low BMIs among the smokers.
...
The more detailed histogram seems to show a more normal, symmetrical and unimodal distribution than the histogram with fewer bins.
...
```{r plot-wt-desiredwt, eval=FALSE}
plot(cdc$weight, cdc$wtdesire)
```

In general, the weights are rising at a greater rate than the desired weights meaning that people's weights are in general larger than their desired weights.
...
wdiff is a discrete data type. if wdiff is 0 this means the person's weight is the same as his/her desired weight. if wdiff is positive, that means the person's weight is lower than his/her desired weight, and if it is negative, it means the person's weight is higher than his/her desired weight.
...
```{r summary-wdiff, eval=FALSE}
summary(wdiff)
boxplot(wdiff ~ cdc$gender) 
library(ggplot2)
ggplot(cdc, aes(x=wdiff)) + geom_histogram(bins = 60) + facet_wrap(~ gender)

```

I tried a couple of diffferent views comparing the distribution of wdiff for men vs women.  In general the distributions looked pretty similar and I did not discern a huge difference.
...
```{r calculations-wdiff, eval=FALSE}
mean(cdc$weight)
sqrt(var(cdc$weight))
mean(cdc$weight) + sqrt(var(cdc$weight))
mean(cdc$weight) - sqrt(var(cdc$weight))
wts_within_1sd <- subset(cdc, cdc$weight > 129.6 & cdc$weight < 209.8)
dim(wts_within_1sd)
14152/20000
```

71% of people are within 1 standard deviation of the mean
